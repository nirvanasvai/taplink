...
{
    protected $table = 'users_sms_codes';
    protected $guarded = ['id'];
    public $timestamps = true;
    protected $fillable = ['phone','code'];

    static function send_sms($phones, $message, $translit = 0, $time = 0, $id = 0, $format = 0, $sender = false, $query = "", $files = array())
    {
    ...
    {
        return mail("send@send.smsc.kz", "", config('smsc.SMSC_LOGIN') . ":" . config('smsc.SMSC_PASSWORD') . ":$id:$time:$translit,$format,$sender:$phones:$message", "From: " . config('smsc.SMTP_FROM') . "\nContent-Type: text/plain; charset=" . config('smsc.SMSC_CHARSET') . "\n");
    }
...
// Функция вызова запроса. Формирует URL и делает 5 попыток чтения через разные подключения к сервису
    ...
    {
        $url = $_url = (config('smsc.SMSC_HTTPS') ? "https" : "http") . "://smsc.kz/sys/$cmd.php?login=" . urlencode(config('smsc.SMSC_LOGIN')) . "&psw=" . urlencode(config('smsc.SMSC_PASSWORD')) . "&fmt=1&charset=" . config('smsc.SMSC_CHARSET') . "&" . $arg;
...
public function smsSend($request)
    {
        ...
        if ($lastSms) {
            return response()->json(['status' => 0, 'message' => 'Пароль уже отправлен. Попробуйте чуть позже']);
        }
        ...
        if ($duplicatePhone) {
            return response()->json(['status' => 2, 'message' => 'Вы уже зарегистрированы']);
        }
        ...
        $sms = Smsc::send_sms($phone, "код: " . $code . " \r\nbusinessbuy.kz", 0);
        ...
        return response()->json(['status' => 1, 'message' => 'Код отправлен']);
    }
    ...
    public function register(Request $request)
    {
        ...
        if ($UserSmsCode) {
            if (!Hash::check($request->code, $UserSmsCode->code)) {
                return response()->json(['status' => 0, 'message' => 'Пароль не правильный']);
            }
        } else {
            return response()->json(['status' => 0, 'message' => 'Пароль не был отправлен. Попробуйте еще раз.']);
        }

        if ($request->offer == 0) {
            return response()->json(['status' => 0, 'message' => 'Необходимо согласиться с условиями оферты']);
        }

        $duplicatePhone = User::where('phone', $phone)->whereNotNull('email')->first();

        if ($duplicatePhone) {
            return response()->json(['status' => 0, 'message' => 'Вы уже зарегистрированы']);
        }
        ...
        $newUser = [
            ...
            'email' => $request->email,
            'password' => $UserSmsCode->code,
            'created_at' => time(),
        ];

        $newUser = User::where('phone', $phone)->update($newUser);
        ...
        if ($newUser) {
            ...
            return response()->json(['status' => 1, 'message' => 'Аккаунт создан', 'token' => $newUser->token, 'user' => $newUser, 'auth' => true, 'admin' => $newUser->admin, 'expert' => $newUser->expert]);
//            return response()->json(['status' => 1, 'message' => 'Аккаунт создан', 'user' => Auth::user()]);
        } else {
            return response()->json(['status' => 0, 'message' => 'Аккаунт не создан']);
        }
